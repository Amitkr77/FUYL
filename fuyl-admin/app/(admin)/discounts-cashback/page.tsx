...
export default async function DiscountCashbackPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-xl font-bold text-slate-900">Discount &amp; Cashback</h1><p className="mt-1 text-sm text-slate-500">Manage customer savings and wallet rewards in one place.</p></div>
      <div className="flex items-center gap-2"><CsvExportButton filename={tab === 'activity' ? 'cashback-activity' : tab === 'cashback' ? 'cashback-policies' : 'discounts'} columns={tab === 'activity' ? [{key:'customerName',label:'Customer'},{key:'amount',label:'Amount'},{key:'status',label:'Status'},{key:'createdAt',label:'Created at'}] : [{key:'name',label:'Name'},{key:'type',label:'Type'},{key:'status',label:'Status'},{key:'startsAt',label:'Starts at'},{key:'endsAt',label:'Ends at'}]} rows={tab === 'activity' ? earnings.items : tab === 'cashback' ? policies : discounts} />{tab !== 'activity' && <Link href={tab === 'cashback' ? '/discounts-cashback/cashback/new' : '/discounts-cashback/discounts/new'} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#558476] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#457366]"><Plus className="h-4 w-4" />{tab === 'cashback' ? 'Create cashback' : 'Create discount'}</Link>}</div>
    </div>
    <TabNav tabs={tabs.map((t) => ({ ...t, href: `/discounts-cashback?tab=${t.id}` }))} activeId={tab} />
