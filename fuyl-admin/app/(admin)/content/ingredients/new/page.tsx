...
import { ArrowLeft, Save, CheckCircle2, AlertCircle, ImagePlus } from 'lucide-react'
import { createIngredientAction, getContentImageUploadSignature } from '../../actions'
...
export default function NewIngredientPage() {
  ...
  const [form, setForm] = useState({
    name: '', amount: '', benefit: '', description: '', image: '',
    category: INGREDIENT_CATEGORIES[0], clinicalBacking: '', isActive: true,
  })
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await createIngredientAction(form)
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">New Ingredient</h2>
            <p className="text-sm text-slate-500">Shown on the storefront&apos;s Ingredients page</p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => set({ name: e.target.value })} placeholder="e.g. Ashwagandha" className={inputCls} />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Amount</label>
                <input type="text" value={form.amount} onChange={(e) => set({ amount: e.target.value })} placeholder="e.g. 300mg" className={inputCls} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Benefit (short tagline)</label>
              <input type="text" value={form.benefit} onChange={(e) => set({ benefit: e.target.value })} placeholder="e.g. Reduces stress & cortisol" className={inputCls} />
            </div>
            <div>
