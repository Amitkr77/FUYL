...
export async function createAffiliateImpersonationAction(id:string):Promise<{error?:string;url?:string}>{try{const{code}=await createAffiliateImpersonation(id);const storefront=(process.env.STOREFRONT_URL??process.env.NEXT_PUBLIC_STOREFRONT_URL??'https://fuyl.in').replace(/\/+$/,'');return{url:`${storefront}/affiliate/impersonate?code=${encodeURIComponent(code)}`}}catch(err){return{error:getErrorMessage(err,'Could not start affiliate login.')}}}
