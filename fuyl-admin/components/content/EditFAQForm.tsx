...
export function EditFAQForm({ faq }: { faq: FAQRecord }) {
  ...
  const [form, setForm] = useState({ question: faq.question, answer: faq.answer, isActive: faq.isActive })
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await updateFAQAction(faq.id, form)
  ...
  const handleDelete = () => {
    startTransition(() => deleteFAQAction(faq.id))
  }
    ...
    <div className="space-y-5 max-w-2xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Edit FAQ</h2>
            <p className="text-sm text-slate-500 truncate max-w-xs">{faq.question}</p>
          </div>
        </div>
