...
import { Input, Select, Checkbox, FormSection } from "@/components/ui/form";
...
export function AffiliateProfileEditor({ affiliate, programs, coupons }: { affiliate: Affiliate; programs: AffiliateProgram[]; coupons: AvailableCoupon[] }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" onMouseDown={() => setEditing(false)}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl" onMouseDown={e => e.stopPropagation()}><div className="flex justify-between"><h3 className="text-lg font-semibold">Edit affiliate</h3><button onClick={() => setEditing(false)} aria-label="Close"><X className="h-5 w-5" /></button></div><form action={submit} className="mt-5 grid gap-4 sm:grid-cols-2">
    <Input label="Name" required name="name" defaultValue={affiliate.name} />
    <Input label="Phone" name="phone" defaultValue={affiliate.phone} />
    <Select label="Affiliate program" required name="programId" defaultValue={selectedProgram} options={programs.filter(p => p.isActive).map(p => ({value:p._id,label:`${p.name} — ${p.commissionType==="flat_per_item"?`₹${p.defaultRate}/item`:p.commissionType==="flat_per_order"?`₹${p.defaultRate}/order`:`${p.defaultRate}%`}`}))} helperText="New orders use this program immediately; historical commissions keep their saved rate." className="sm:col-span-2" />
    <Input label="Promotion channels" name="channels" defaultValue={affiliate.channels.join(", ")} className="sm:col-span-2" />
    ...
    <Input label="UPI ID" name="upi" defaultValue={affiliate.paymentInfo?.upi} />
    <Input label="Account holder" name="accountName" defaultValue={affiliate.paymentInfo?.accountName} />
    <Input label="Bank account" name="bankAccount" defaultValue={affiliate.paymentInfo?.bankAccount} />
    <Input label="IFSC" name="ifsc" defaultValue={affiliate.paymentInfo?.ifsc} />
    {error && <p className="sm:col-span-2 rounded-lg bg-red-50 p-2 text-sm text-red-600">{error}</p>}<div className="flex justify-end gap-2 sm:col-span-2"><button type="button" onClick={() => setEditing(false)} className="rounded-lg border px-4 py-2 text-sm">Cancel</button><button disabled={saving} className="rounded-lg bg-[#558476] px-4 py-2 text-sm font-semibold text-white hover:bg-[#457366] disabled:opacity-50">{saving ? "Saving..." : "Save changes"}</button></div>
  </form></div></div>;
}
