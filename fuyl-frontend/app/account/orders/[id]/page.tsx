...
function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-brand-border p-5 sm:p-6 ${className}`}
    >
      {children}
    </div>
  );
...
export default function OrderDetailPage() {
  ...
  if (!user) {
    return (
      <div className="container-brand section-py text-center">
        <p className="text-display-md font-display mb-4">
  ...
  return (
    <div>
      <Link
        href="/account/orders"
        className="inline-flex items-center gap-1.5 text-body-sm mb-6 text-brand-muted hover:text-brand-forest transition-colors"
      >
      ...
      {!isLoading && error && (
        <p
          className="text-body-sm p-3 rounded-sm"
      ...
      {!isLoading && !error && order && (
        <div className="animate-fade-in">
          ...
          {(order.status === "cancelled" || order.status === "returned") && (
            <div
              className="flex items-start gap-2.5 p-4 rounded-2xl mb-8"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                        ...
                        <div className="flex items-center gap-4">
                          {item.image ? (
                            ...
                            <div className="w-16 h-16 rounded-lg bg-brand-sage/40 flex items-center justify-center shrink-0">
                              <Package
                                size={20}
                                className="text-brand-forest/50"
                              />
                            </div>
                          )}
                          ...
                          <p className="text-body-sm font-medium text-brand-forest">
              ...
              <Card>
                ...
                {order.trackingUrl && (
                  <a
                    ...
                    className="text-body-sm text-brand-teal font-semibold hover:underline mt-1 inline-block"
                  >
                    Track package →
                  </a>
                )}
                {order.shippedAt && (
                  <p className="text-body-xs text-brand-muted mt-2">
                  ...
                  <div className="relative flex flex-col gap-0">
                    ...
                    {order.timeline.map((event, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`relative flex items-start gap-3.5 ${isFirst ? "" : "pt-4"}`}
                        >
                          <span
                            ...
                            style={{
                              background: m?.dot ?? "#9CA3AF",
                              boxShadow: "0 0 0 2px " + (m?.bg ?? "#F3F4F6"),
                            }}
                          >
            ...
            <div className="space-y-6 lg:sticky lg:top-6 h-fit">
              {/* Summary */}
              <Card>
                <p className="text-label text-brand-muted mb-4">
                  Order Summary
                </p>
                <div className="flex flex-col gap-2">
              ...
              <Card>
                ...
                {((payments[0]?.reference ?? order.razorpayPaymentId) ||
                  ...
                  (payments[0]?.refundedAmount ?? 0) > 0) && (
                  <div className="mt-3 pt-3 border-t border-brand-border space-y-1">
                    {(payments[0]?.reference ?? order.razorpayPaymentId) && (
                      <p className="text-body-xs text-brand-muted break-all">
                        Txn ID:{" "}
                        {payments[0]?.reference ?? order.razorpayPaymentId}
                      </p>
                    )}
                    {payments[0]?.capturedAt && (
                      <p className="text-body-xs text-brand-muted">
                        Paid on {formatDateTime(payments[0].capturedAt)}
                      </p>
                    )}
                    {payments[0] && payments[0].refundedAmount > 0 && (
                      <p className="text-body-xs text-brand-muted">
                        Refunded: {formatPrice(payments[0].refundedAmount)}
                      </p>
                    )}
                  </div>
                )}
